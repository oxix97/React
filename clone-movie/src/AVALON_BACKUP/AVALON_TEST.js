  ...
  if (page === MAIN_SELECT_PLAYER) {
    return (
      <div>
        <h3>
          {"원정에 참여하는 인원 수 : " +
            game.takeStage[game.expeditionStage] +
            "명"}
        </h3>
        {user.map((user, index) => (
          <ul key={index}>
          ...
          {user.map((user, index) => (
            <User key={index}>
              ...
              {index === game.represent ? (
                <button onClick={() => setPage(MAIN_SELECT_PLAYER)}>
                  원정 인원 정하기
                </button>
              ) : null}
            </User>
  ...
  if (page === VOTE_RESULT) {
    return (
      <div>
        ...
        <button onClick={() => votePage(setPage(EXPEDITION_FRAME))}>
          다음
        </button>
      </div>
    );
    ...
    return (
      <>
        <div>
          ...
          <button onClick={() => expeditionClick(setPage(EXPEDITION_RESULT))}>
            Click
          </button>
        </div>
      </>

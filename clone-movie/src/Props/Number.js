import PropTypes from "prop-types";

function Number({ name, picture, rating }) {
  return (
    <div>
      <h1>I like {name}</h1>
      <h4>{rating} / 5.0</h4>
      <img width="200px" height="200px" src={picture} alt={name} />
    </div>
  );
}

const NumberLike = [
  {
    id: "1",
    name: "Number1",
    image:
      "data:image/png;base64,",
    rating: 5.0,
  },
  {
    id: "2",
    name: "Number2",
    image:
      "https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/NYCS-bull-trans-2.svg/1200px-NYCS-bull-trans-2.svg.png",
    rating: 4.8,
  },
  {
    id: "3",
    name: "Number3",
    image:
      "https://upload.wikimedia.org/wikipedia/commons/thumb/2/25/NYCS-bull-trans-3.svg/600px-NYCS-bull-trans-3.svg.png",
    rating: 4.5,
  },
];

Number.propTypes = {
  name: PropTypes.string.isRequired,
  picture: PropTypes.string.isRequired,
  rating: PropTypes.number,
};

const renderNumber = (number) => (
  <Number
    key={number.id}
    name={number.name}
    picture={number.image}
    rating={number.rating}
  />
);

function PropsNumber() {
  console.log(NumberLike.map(renderNumber));
  return <div>{NumberLike.map(renderNumber)}</div>;
}
export default PropsNumber;

...
import { PageFrame, UserList, GameFrame, GameState } from "./Styled";
...
function MainFrame() {
  ...
  const angels = gameState.takeStage.filter(
    (element) => "success" === element
  ).length;
  const evils = gameState.takeStage.filter(
  ...
  return (
    <PageFrame>
      {angels === 3 ? (
        playerState.map((e) => e.role === "Assassin") ? (
          <AssassinPlayer />
        ) : null
      ) : null}

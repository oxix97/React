...
import {
  ...
  Frame,
  VoteStageFrame,
} from "../AVALON_BACKUP/MainPage/Styled";
import {
  GameStart,
  SetPage,
  VoteOnChange,
...
function VIEW() {
  ...
  if (context.page === Pages.START_FRAME) {
    return <button onClick={GameStart}>게임 시작</button>;
  }
          ...
          {context.playerData.map((user, index) => (
            <User key={index}>
              ...
              {index === context.represent ? (
                <button onClick={() => SetPage(Pages.MAIN_VOTE)}>
                  원정 인원 정하기
                </button>
              ) : null}
            </User>
  ...
  if (context.page === Pages.MAIN_VOTE) {
    return (
      <div>
        <h3>
          {"원정에 참여하는 인원 수 : " +
            context.takeStage[context.expeditionStage] +
            "명"}
        </h3>
        {context.playerData.map((user, index) => (
          <ul key={index}>
      ...
      <div>
        <div>
          {context.expeditionStage === 4 && context.playerData.length >= 7 ? (
            <div>
              {context.vote.filter((element) => "fail" === element).length >= 2
                ? "원정 실패"
                : "원정 성공"}
              <div>
                성공 개수 :{" "}
                {context.vote.filter((element) => "success" === element).length}
              </div>
              <div>
                실패 개수 :
                {context.vote.filter((element) => "fail" === element).length}
              </div>
            </div>
          ) : (
            <div>

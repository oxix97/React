...
import {
  ...
  mustHaveRoles,
  needPlayers,
} from "../AVALON_BACKUP/gameSetting";
...
export const GameStart = () => {
  ...
  if (PlayersNumber >= 5) {
    const temp = [...mustHaveRoles, ...expandRoles.slice(0, PlayersNumber - 5)];
...
export const VotePage = () => {
  const [state, dispatch] = useReducer(reducer, initContext);
  const agree = state.playerData.filter(
    (element) => "agree" === element
  ).length;
  const oppose = state.playerData.filter(
    (element) => "oppose" === element
  ).length;
  let expeditionStage,
    index = state.expeditionStage;
  let page = "";
  let voteStage = 0;
...
export const ExpeditionClick = () => {
  ...
  if (state.expeditionStage === 4 && state.playerData.length >= 7) {
    ...
    state.vote.includes("fail") ? (value = "fail") : (value = "success");
  }

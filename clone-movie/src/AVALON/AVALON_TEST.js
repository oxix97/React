  ...
  switch (action.type) {
    ...
    case "playerCheckedNumber":
      return {
        ...state,
        playerCheckedNumber:
          state.playerCheckedNumber + action.playerCheckedNumber,
      };
    case "checkedReset":
...
const AVALON_TEST = ({ Store }) => {
  ...
  const nextPage = () => {
    ...
    const angelCount = game.takeStage.filter(
      (element) => "success" === element
    ).length;
    const evilCount = game.takeStage.filter(
  ...
  if (state.page === START_FRAME) {
    ...
    return <button onClick={() => gameStart(data)}>게임 시작</button>;
  }
        ...
        {user.map((user, index) => (
          <User key={index}>
            ...
            {index === game.represent ? (
              <button onClick={mainFrameClicked}>원정 인원 정하기</button>
            ) : null}
          </User>
        ))}
      </>
      ...
      <div>
        ...
        {!state.voteResult ? (
          <div>
            ...
            <button
              onClick={() => dispatch({ type: "voteResult", voteResult })}
            >
              결과
            </button>
          </div>
          ...
          <div>
            ...
            <button
              onClick={() =>
                votePage(dispatch({ type: "voteResult", voteResult }))
              }
            >
              다음
            </button>
        ...
        {!state.expedition ? (
          ...
          <div>
            {game.expeditionStage === 4 && user.length >= 7 ? (
              <div>
                {game.vote.filter((element) => "fail" === element).length >= 2
                  ? "원정 실패"
                  : "원정 성공"}
                <div>
                  성공 개수 :{" "}
                  {game.vote.filter((element) => "success" === element).length}
                </div>
                <div>
                  실패 개수 :
                  {game.vote.filter((element) => "fail" === element).length}
                </div>
              </div>
            ) : (
              <div>
            ...
            <button onClick={() => nextPage(dispatch({ type: "page", page }))}>
              다음
            </button>
          </div>
        )}
